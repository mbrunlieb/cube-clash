...
function createGame() {
  ...
  const game = {
    ...
    status: "waiting",
    players: {},
    decks: { A: null, B: null }, // updated deck from client editor
    state: {
      playerA: {
        seat: "A",
...
function findCardInZones(playerState, instanceId) {
  for (const zone of ["battlefield", "hand", "graveyard", "exile", "library"]) {
    const idx = playerState[zone].findIndex(c => c.instanceId === instanceId);
    if (idx !== -1) return { zone, idx, card: playerState[zone][idx] };
  }
  return null;
}
...
io.on("connection", (socket) => {
  ...
  socket.on("update_deck", ({ gameId, seat, cards }) => {
    ...
    if (!game) return;
    // Rebuild the deck state for this seat with the updated cards
  ...
  socket.on("game_action", ({ gameId, action }) => {
    ...
    switch (action.type) {
      ...
      case "RETURN_TO_HAND": {
        const card = removeFromZone(ps, "graveyard", action.instanceId);
        if (card) ps.hand.push(card);
        break;
      }
      ...
      case "CONCEDE": {
        ...
        io.to(gameId).emit("game_over", { winner: oppSeat, reason: `${player.name} conceded.` });
        break;
      }
